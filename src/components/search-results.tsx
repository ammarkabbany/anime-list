import { useState, useEffect, type ChangeEventHandler, type SetStateAction, type Dispatch } from "react";
...
export default function SearchResults({ query, setQuery }: { query: string, setQuery: Dispatch<SetStateAction<string>> }) {
  ...
  return (
    <div className="mx-auto max-w-xs sm:max-w-sm"> {/* Reduced space-y for tighter layout if card has margin */}
      {(isLoading || results.length > 0) && (
        <Card className="rounded-xl shadow-xl border border-primary/20 transition-all duration-300 ease-in-out mt-2 py-1">
              ...
              results.map((result) => (
                ...
                >
                  ...
                  <div className="space-y-0.5"> {/* Added space-y for consistent spacing */}
                    ...
                    <h4 className="font-semibold text-xs text-muted-foreground group-hover:text-primary line-clamp-2">{result.title_english}</h4>
